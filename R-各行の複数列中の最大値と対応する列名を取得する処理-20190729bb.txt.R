...
#####################################
## 最大値関数の引数となる列のみに絞ったdfに欠損値がある場合
	# dreplace(df2.targetvars, is.na(df2.targetvars), -Inf) を用いて欠損値を -Infに置換する
## 対応前 j2 <- max.col(df2.targetvars , "first") #1.「各行で最大値を持つ列番号を取得」
## 対応後 j2 <- max.col(replace(df2.targetvars, is.na(df2.targetvars), -Inf) , "first")